...
function Master.pushRotateScale(x, y, r, sx, sy)
    love.graphics.push()
...
function Master:new(parent, x, y, opts)
    ...
    self.rv = 1.23 * math.pi -- velocidad de impulso o de giro
    self.v = 0               -- velocidad
    --self.max_v = 10 --100
    self.base_max_v = 35
    self.a = 40 --100 -- aceleración
        ...
        function()
            ...
            local trail = TrailParticle(
                ...
                self.y - 0.9 * self.w * math.sin(self.r) + 0.2 * self.w * math.sin(self.r - math.pi / 2),
                {
                    ...
                    r = utils.random(3, 6),          -- radio de la bola de fuego
                    rMinus = utils.random(0.2, 0.4), -- disminución progresiva del radio
                    d = utils.random(0.10, 0.35),
...
function Master:destroy()
    if not self.parent or not self.parent.timer then self.dead = true; self:destroy(); end
...
function Master:toBoostDown(dt)
    if self:canBoost() then
...
function Master:draw()
    if self.dead then return end
    Master.pushRotateScale(self.x, self.y, self.r, 1, 1)
        love.graphics.setColor(self.color or default_color)
        -- shipName = "Master, Master, Medium, Tulip, Crusader"
