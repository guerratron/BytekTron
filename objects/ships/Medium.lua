...
function Medium.pushRotateScale(x, y, r, sx, sy)
    love.graphics.push()
...
function Medium:new(parent, x, y, opts)
    ...
    self.rv = 0.53 * math.pi -- velocidad de impulso o de giro
    self.v = 0               -- velocidad
    --self.max_v = 10 --100
    self.base_max_v = 75
    ...
    self.shoot_timer = 0
    self.shoot_cooldown = 0.10 -- lo trae el ataque no el tipo de nave
        ...
        function()
            ...
            local trail = TrailParticle(
                ...
                self.y - 0.9 * self.w * math.sin(self.r),
                {
                    ...
                    r = utils.random(4, 8),          -- radio de la bola de fuego
...
function Medium:toBoostDown(dt)
    if self:canBoost() then
...
function Medium:draw()
    if self.dead then return end
    Medium.pushRotateScale(self.x, self.y, self.r, 1, 1)
