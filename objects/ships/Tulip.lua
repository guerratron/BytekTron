...
function Tulip.pushRotateScale(x, y, r, sx, sy)
    love.graphics.push()
...
function Tulip:new(parent, x, y, opts)
    ...
    self.rv = 0.93 * math.pi -- velocidad de impulso o de giro
    self.v = 0               -- velocidad
    --self.max_v = 10 --100
    self.base_max_v = 45
    self.a = 35 --100 -- aceleración
    ...
    self.boost_cooldown = 4 -- sg después de llegar a boost = 100 se permitirá de nuevo el can_boost
    ...
    self.handler1 = self.parent.timer:every(
        ...
        function()
            ...
            local c = utils.tableRandom(negative_colors)
            ...
            local trail = TrailParticle(
                ...
                self.y - 0.9 * self.w * math.sin(self.r) + 0.2 * self.w * math.sin(self.r - math.pi / 2),
                {
                    ...
                    r = utils.random(2, 3),          -- radio de la bola de fuego
                    rMinus = utils.random(0.1, 0.3), -- disminución progresiva del radio
    ...
    for i = 1, utils.random(numFragments * 0.6, numFragments * 1) do -- número de fragmentos
        ...
        local eff2 = ExplodeParticle(
            ...
            self.y,
            {
                ...
                s = 1, -- semi-longitud de los fragmentos
                v = 40,
...
function Tulip:update(dt)
    ...
    self.boost = math.min(self.boost + 4 * dt, self.max_boost)
...
function Tulip:draw()
    if self.dead then return end
    Tulip.pushRotateScale(self.x, self.y, self.r, 1, 1)
        love.graphics.setColor(self.color or default_color)
        -- shipName = "Tulip, Tulip, Medium, Tulip, Crusader"
